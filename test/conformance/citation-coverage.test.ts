...
describe('Citation Coverage', () => {
  ...
  beforeAll(() => {
    ...
    try {
      ...
      console.warn('[citation-coverage.test] Fixture files not found, skipping tests:', (e as Error).message);
    }
  });
  ...
  describe('Combined Coverage', () => {
    ...
    it('should maintain T:XXX format for Atrium within range 1-618', () => {
      if (!hasFixtures) return;
      const atriumRefs = atriumLinker.toRegistry().references;
      for (const ref of Object.values(atriumRefs)) {
        expect(ref.displayId).toMatch(/^T:\d+$/);
        expect(ref.entityCode).toBe('5130.KL');
        // Verify numeric part is within documented range 1-618
        const numericPart = parseInt(ref.displayId.split(':')[1], 10);
        expect(numericPart).toBeGreaterThanOrEqual(1);
        expect(numericPart).toBeLessThanOrEqual(618);
      }
    });

    it('should maintain A:XXX format for Axis within range 1-352', () => {
      if (!hasFixtures) return;
      const axisRefs = axisLinker.toRegistry().references;
      for (const ref of Object.values(axisRefs)) {
        expect(ref.displayId).toMatch(/^A:\d+$/);
        expect(ref.entityCode).toBe('5106.KL');
        // Verify numeric part is within documented range 1-352
        const numericPart = parseInt(ref.displayId.split(':')[1], 10);
        expect(numericPart).toBeGreaterThanOrEqual(1);
        expect(numericPart).toBeLessThanOrEqual(352);
      }
    });
  });
