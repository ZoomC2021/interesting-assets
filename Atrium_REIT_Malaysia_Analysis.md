# Atrium REIT (Malaysia) - Comprehensive Investment Analysis

**Stock Code:** 5130.KL (Bursa Malaysia) [T:1]
**Research Date:** April 2026 [T:2]
**Data Coverage:** FY 2025 (December 31, 2025) [T:3]
**Report Status:** Based on 2025 Annual Report published March 26, 2026 [T:4]

---

## Executive Summary

Atrium Real Estate Investment Trust (Atrium REIT) [T:5] is a Malaysian industrial REIT focused on warehouse and manufacturing properties primarily located in the Klang Valley (Selangor) and Penang [T:6]. As of December 31, 2025 [T:7], the REIT maintains a portfolio of 9 industrial properties [T:8] with a total asset value of RM 723.24 million [T:9].

**Key Highlights (FY 2025):**
- **Portfolio Occupancy:** 100% [T:10] (all properties fully occupied as of Q4 2025) [T:11]
- **Gearing Ratio:** ~43.5% [T:12] (comfortably below 60% regulatory limit [T:13])
- **Interest Coverage:** ~2.09x [T:14] (adequate coverage above 2.0x threshold)
- **2025 Total DPU:** 9.30 sen [T:15] (distributed across 5 payments [T:16])
- **NAV per Unit:** RM 1.3874 [T:17] (before final distribution)
- **Full Year Gross Revenue:** RM 51.05 million [T:18]
- **Full Year NPI:** RM 45.98 million [T:19]

---

## 1. Company Overview

### 1.1 Corporate Structure
| Attribute | Details |
|-----------|---------|
| **Manager** | Atrium REIT Managers Sdn Bhd [T:20] |
| **Trustee** | Maybank Trustees Berhad [T:21] |
| **Property Manager** | Atrium REIT Managers Sdn Bhd [T:22] |
| **Stock Exchange** | Bursa Malaysia (Main Market) [T:23] |
| **Sector** | Real Estate Investment Trusts (REITs) [T:24] |
| **Property Focus** | Industrial (warehouses, manufacturing facilities) [T:25] |

### 1.2 Portfolio Composition
| Metric | Value (Dec 2025) |
|--------|------------------|
| **Total Assets** | RM 723.24 million [T:26] |
| **Investment Properties** | RM 684.97 million [T:27] |
| **Number of Properties** | 9 industrial buildings [T:28] |
| **Geographic Focus** | Klang Valley (7 properties) + Penang (2 properties) [T:29] |

### 1.2 REIT Manager Deep Dive: Atrium REIT Managers Sdn Bhd ⚠️

#### Ownership & Control Structure (🔴 Governance Concern)
| Aspect | Details | Risk Level |
|--------|---------|------------|
| **Established** | 2005 [T:30] | - |
| **Controlling Shareholder** | Chan Kam Tuck via Glory Blitz Industries Sdn Bhd [T:31] | 🔴 **59% ownership** [T:32] |
| **REIT Unitholding** | Chan Kam Tuck holds ~23.7% of units directly and through related entities [T:33] | 🔴 **Concentration** |
| **Governance Issue** | Single family/entity controls both Manager AND holds largest unitholder stake [T:34] | 🔴 **Conflict of interest risk** |

**⚠️ Critical Governance Concern:** Chan Kam Tuck's effective control of both the Manager (59%) [T:35] and his ~24% unitholder stake [T:36] creates potential conflicts of interest. Decisions may favor the controlling family over minority unitholders.

#### Key Executives & Management Team
| Name | Position | Background | Tenure | Status |
|------|----------|------------|--------|--------|
| **Dato' Dr. Ir. Mohamad Khir Bin Harun** [T:37] | Chairman (Non-Independent Non-Executive) | Engineer (BSc Brighton Polytechnic, MSc Brunel, PhD Manchester), 40+ years in communications industry [T:38] | Since Oct 2009 [T:39] | Stable |
| **Lee Hooi Hoong** [T:40] | Acting CEO & CFO | CIMA qualified, 30+ years experience, joined Oct 2007 as CFO [T:41] | 17+ years [T:42] | ⚠️ **Dual role** |
| **Wong Sui Ee** [T:43] | Executive Director, Head of Investment | ICAEW qualified, former KPMG LLP (UK), former AVP at Perdana Merchant Bank Bhd [T:44] | Long-standing | Stable |
| **Foong Wai Leng** [T:45] | Executive Director, Head of Risk & Compliance | Appointed June 2022 [T:46] | Since 2022 | Newer |

#### Recent Management Disruption (August 2024) ⚠️
- **Chan Kum Chong retired as CEO effective August 31, 2024** [T:47] (brother of Chan Kam Tuck [T:48])
- **Lee Hooi Hoong (CFO) now serves as Acting CEO** while continuing CFO duties [T:49]
- Creates **role concentration concerns** and succession planning uncertainty

#### Board Governance
| Metric | Atrium REIT | Best Practice | Assessment |
|--------|-------------|---------------|------------|
| **Board Size** | 6 directors [T:50] | 6-8 typical | Adequate |
| **Independent Directors** | 3 of 6 (50%) [T:51] | Majority preferred | ⚠️ **Below best practice** |
| **Committees** | 2 (Audit, Nominating) [T:52] | 3-4 typical | ⚠️ **Limited** |
| **Remuneration Committee** | ❌ Not disclosed | Should exist | 🔴 **Gap** |

#### Fee Structure
| Fee Type | Rate | Notes |
|----------|------|-------|
| **Base Management Fee** | Up to **1.0%** p.a. of NAV [T:53] | May elect lower rate |
| **Acquisition Fee** | **1.0%** of purchase [T:54] | Standard |
| **Disposal Fee** | **0.5%** of sale [T:55] | Standard |
| **Performance Fee** | ❌ **Not disclosed** | No DPU alignment |

#### Manager Assessment
| Category | Rating | Rationale |
|----------|--------|-----------|
| **Ownership Structure** | 🔴 Poor | High concentration, family control |
| **Management Stability** | 🟡 Concerning | Recent CEO retirement, dual role |
| **Governance Practices** | 🟡 Below Standard | 50% independence, limited committees |

### 1.3 Property List
| Property Name | Location | Type | Status |
|---------------|----------|------|--------|
| Atrium Shah Alam 1 (ASA1) [T:56] | Shah Alam, Selangor [T:57] | Industrial | Operational |
| Atrium Shah Alam 2 (ASA2) [T:58] | Shah Alam, Selangor | Industrial | Operational |
| Atrium Shah Alam 3 (ASA3) [T:59] | Shah Alam, Selangor | Industrial | Operational |
| Atrium Shah Alam 4 (ASA4) [T:60] | Shah Alam, Selangor | Industrial | Operational |
| Atrium Shah Alam 5 (ASA5) [T:61] | Shah Alam, Selangor | Industrial | Operational (AEI completed) [T:62] |
| Atrium Puchong [T:63] | Puchong, Selangor [T:64] | Industrial | Operational |
| Atrium USJ [T:65] | USJ, Selangor [T:66] | Industrial | Operational |
| Atrium Bayan Lepas 1 [T:67] | Penang [T:68] | Industrial | Operational |
| Atrium Bayan Lepas 2 [T:69] | Penang | Industrial | Operational |

### 1.4 Detailed Property Specifications (Discovered via Research)

| Property | Specific Address | NLA (sq ft) | Valuation (RM) | Tenant | Lease Term |
|----------|----------------|-------------|----------------|--------|------------|
| **Atrium Shah Alam 1 (ASA1)** | Lot 1-8, Persiaran Jubli Perak, Seksyen 22, Shah Alam [T:70] | 311,736 [T:71] | RM 111,000,000 [T:72] | **Lazada Express** (historical) / Current unclear | — |
| **Atrium Shah Alam 2 (ASA2)** | Lot 7A, Persiaran Jubli Perak, Seksyen 22, Shah Alam [T:73] | Not disclosed | Not itemized | **Samsung SDS Malaysia Sdn Bhd** [T:74] | 2-year renewal (2024) [T:75] |
| **Atrium Shah Alam 3 (ASA3)** | Shah Alam, Selangor | ~31,544 [T:76] | Not itemized | **DSV Solutions (DC) Sdn Bhd** ⭐ NEW [T:77] | 3-year renewal (early 2024) [T:78] |
| **Atrium Shah Alam 4 (ASA4)** | Shah Alam, Selangor | ~124,766 [T:79] | Not itemized | **DHL Properties (Malaysia) Sdn Bhd** ⭐ NEW [T:80] | Lease commenced Jan 2024 [T:81] |
| **Atrium Shah Alam 5 (ASA5)** | Shah Alam, Selangor | ~41,000-48,000 [T:82] | Not itemized | **New tenant (undisclosed)** [T:83] | 3-year lease, commenced Sep 2025 [T:84] |
| **Atrium Puchong** | Puchong, Selangor | Not disclosed | Not itemized | **Lazada Express (Malaysia) Sdn Bhd** ⭐ NEW [T:85] | — |
| **Atrium USJ** | USJ, Selangor | Not disclosed | Not itemized | **Röhlig Malaysia Sdn Bhd** (Block B) [T:86] | 3-year renewal (2024) [T:87] |
| **Atrium Bayan Lepas 1** | Penang | Not disclosed | RM 48,000,000 [T:88] | **Lumileds Malaysia Sdn Bhd** [T:89] | 15-year lease (2019-2034) [T:90] |
| **Atrium Bayan Lepas 2** | Penang | Not disclosed | RM 136,600,000 [T:91] | **Lumileds Malaysia Sdn Bhd** | 15-year lease (2019-2034) |

**Property Type:** All properties are single-storey warehouses annexed with 2-4 storey office buildings [T:92].
**Land Area (ASA1 only):** 573,124 sq ft [T:93]
**Total Known Valuations:** RM 295.6m (ASA1 + Bayan Lepas 1&2) = ~43% of total portfolio [T:94]

---

## 2. Asset Quality Analysis

### 2.1 Occupancy Performance
| Period | Occupancy Rate | Key Events |
|--------|----------------|------------|
| Q3 2025 | ~90-95% [T:95] | ASA5 under AEI until September 11, 2025 [T:96] |
| Q4 2025 | **100%** [T:97] | ASA5 fully tenanted; full portfolio occupied [T:98] |

**Assessment:** The 100% occupancy rate as of Q4 2025 is excellent and demonstrates strong tenant demand for Atrium REIT's industrial properties in prime locations.

### 2.2 Tenant Quality
| Factor | Assessment |
|--------|------------|
| **Tenant Profile** | Primarily multinational corporations (MNCs) and established local industrial operators |
| **Lease Structure** | Long-term leases with built-in rental escalation clauses |
| **Credit Quality** | MNC tenant base provides payment reliability and creditworthiness |
| **Industry Diversification** | Mixed industrial tenants reducing single-industry concentration risk |

### 2.3 Property Enhancements (2025)
| Project | Investment | Status | Impact |
|---------|------------|--------|--------|
| **ASA5 Asset Enhancement** | RM 32.35 million [T:99] | Completed Q3 2025 [T:100] | New tenancy commenced September 11, 2025 [T:101] |
| **ASA4 Enhancements** | RM 2.09 million [T:102] | Completed [T:103] | Maintained asset competitiveness |
| **Total 2025 Capex** | RM 34.44 million [T:104] | Completed | Enhanced portfolio value and income potential |

**Assessment:** The RM 34.44 million capital expenditure in 2025 demonstrates proactive asset management to maintain competitiveness and attract quality tenants.

### 2.4 Asset Valuation
| Metric | Value (Dec 2025) |
|--------|------------------|
| **Investment Properties** | RM 684.97 million [T:105] |
| **Valuation Method** | Independent professional valuation per Malaysian REIT guidelines [T:106] |
| **Valuation Frequency** | Annual revaluation [T:107] |

### 2.5 Geographic Concentration Analysis
| Region | Property Count | Percentage of Portfolio |
|--------|---------------|------------------------|
| **Klang Valley (Selangor)** | 7 [T:108] | ~78% [T:109] |
| **Penang** | 2 [T:110] | ~22% [T:111] |

**Risk Assessment:** High geographic concentration in the Klang Valley presents both opportunity (proximity to Port Klang, major logistics hub) and risk (regional economic downturn impact).

### 2.6 Asset Age Profile & Portfolio Vintage ⭐ NEW

**Property-by-Property Acquisition Timeline:**
| Property | Acquisition Date | Purchase Price | Year Built | Building Age (2026) | Notes |
|----------|------------------|----------------|------------|---------------------|-------|
| **Atrium Shah Alam 1** | 2 April 2007 (IPO) [T:112] | Part of IPO | 2005 [T:113] | **21 years** [T:114] | Original portfolio |
| **Atrium Shah Alam 2** | 2 April 2007 (IPO) [T:115] | Part of IPO | Mid-2000s [T:116] | ~18-20 years [T:117] | AEI 2016 (RM 15.4M) [T:118] |
| **Atrium Shah Alam 4** | 2 April 2007 (IPO) [T:119] | Part of IPO | 1990 [T:120] | **36 years** [T:121] | AEI 2023 (DHL specs) [T:122] |
| **Atrium Puchong** | 2 April 2007 (IPO) [T:123] | Part of IPO | Early 2000s [T:124] | ~20+ years [T:125] | Original portfolio |
| **Atrium USJ** | 22 Dec 2011 [T:126] | Not disclosed | 2008 [T:127] | **18 years** [T:128] | Subang Jaya expansion |
| **Atrium Shah Alam 3** | 16 Mar 2017 [T:129] | RM 23.0M [T:130] | Pre-2017 | N/A | AEI 2018 (RM 9.65M) [T:131] |
| **Atrium Bayan Lepas 2** | 7 Oct 2019 [T:132] | RM 130M [T:133] | 2012 [T:134] | **14 years** [T:135] | Sale-and-leaseback (Lumileds) |
| **Atrium Bayan Lepas 1** | 22 Oct 2020 [T:136] | RM 50M [T:137] | **1972** [T:138] | **54 years** [T:139] | Oldest asset ⚠️ |
| **Atrium Shah Alam 5** | 6 May 2024 [T:140] | RM 41M [T:141] | Pre-2024 | N/A | AEI completed Sep 2025 [T:142] |

**Asset Age Composition:**
| Vintage Category | Properties | % of Portfolio | Characteristics |
|------------------|------------|----------------|-----------------|
| **IPO Vintage (2007)** | 4 properties (ASA1, ASA2, ASA4, Puchong) [T:143] | **44%** [T:144] | ~18-36 years old; 50% have undergone AEIs |
| **Expansion Phase (2011-2017)** | 2 properties (USJ, ASA3) [T:145] | **22%** [T:146] | ~8-15 years since acquisition; ASA3 AEI-refreshed |
| **Penang Expansion (2019-2020)** | 2 properties (Bayan Lepas 1&2) [T:147] | **22%** [T:148] | Recent acquisitions; Bayan Lepas 1 is 54-year-old building |
| **Most Recent (2024-2025)** | 1 property (ASA5) [T:149] | **11%** [T:150] | Newest acquisition with fresh AEI (Sep 2025) |

**AEI (Asset Enhancement Initiative) History:**
| Property | AEI Year | Investment | Nature of Works | Effective Age Reset? |
|----------|----------|------------|-----------------|---------------------|
| **ASA2** | 2016 [T:151] | RM 15.4M [T:152] | Floor/roof replacement, lighting, sheet piling [T:153] | ✅ Functionally newer |
| **ASA3** | 2018 [T:154] | RM 9.65M [T:155] | Piling, raised floor, height increase, loading bays [T:156] | ✅ Functionally newer |
| **ASA4** | 2023 [T:157] | RM 2.09M [T:158] | Refurbishment to DHL specifications [T:159] | ✅ Customized for tenant |
| **ASA5** | 2025 [T:160] | RM 32.35M [T:161] | Major renovation for new tenant [T:162] | ✅ Brand new effective age |

**Key Age Insights:**
- ⚠️ **Oldest Asset:** Bayan Lepas 1 (built 1972, **54 years old**) [T:163] - significant age; may require future capex attention despite current Lumileds tenancy
- ✅ **AEI Strategy:** 4 of 9 properties (44%) [T:164] have undergone major AEIs since 2016 [T:165], effectively resetting functional asset age
- ⚠️ **IPO Concentration:** 44% of portfolio [T:166] dates to 2007 IPO - higher vintage concentration than Axis REIT
- ✅ **Youngest Effective Assets:** ASA3, ASA4, ASA5 (all post-AEI) [T:167] represent modern, customized facilities

**Comparison to Axis REIT:**
| Factor | Atrium REIT | Axis REIT |
|--------|-------------|-----------|
| **Portfolio Size** | 9 properties [T:168] | 69-70 properties [T:169] |
| **IPO Vintage %** | **44%** (4/9 from 2007) [T:170] | Lower concentration across 20-year history |
| **AEI Activity** | Active (4 recent AEIs) | Very active (brownfield focus) [T:171] |
| **Oldest Asset** | **54 years** (Bayan Lepas 1, 1972) [T:172] | Similar vintage range but better diversified |
| **Age Transparency** | Limited disclosure | No weighted average age disclosed |
| **Capex Strategy** | Property-by-property AEI | Systematic capital recycling + development [T:173] |

**Assessment:**
Atrium REIT's 9-property portfolio has **higher age concentration risk** than Axis REIT, with 44% dating to the 2007 IPO and one asset (Bayan Lepas 1) being 54 years old. However, management has been proactive with **4 major AEIs (2016-2025)** that have effectively modernized key assets. The AEI strategy is critical for maintaining competitiveness given the portfolio's concentrated vintage.

**Investor Implication:** The portfolio requires continued AEI investment to remain competitive. Bayan Lepas 1's 54-year age is a long-term capex consideration, though the 15-year Lumileds lease (through 2034) provides near-term income stability. ASA3, ASA4, and ASA5 represent the most modern effective assets post-enhancement.

### 2.7 Tenant Concentration Analysis

#### 2.7.1 Tenant Profile Overview
| Factor | Details |
|--------|---------|
| **Primary Tenant Type** | Multinational corporations (MNCs) in logistics, warehousing, and manufacturing |
| **Secondary Tenant Type** | Established local industrial operators |
| **Average Tenant Quality** | Investment-grade to strong corporate credits |
| **Lease Structure** | Long-term triple-net or equivalent leases with built-in escalations |

#### 2.7.2 Top Tenant Disclosure
| Metric | Status | Notes |
|--------|--------|-------|
| **Top 5 Tenants by Rental Income** | **Partially Disclosed** | **6 of 9** property tenants identified via research |
| **Top Tenant % Contribution** | **Not Disclosed** | Competitive/confidentiality reasons |
| **Tenant Sector Breakdown** | **Partial** | All confirmed tenants are **logistics/e-commerce**: DHL, DSV, Lazada, Röhlig, Samsung SDS |
| **Tenant Credit Ratings** | **Not Available** | Individual tenant ratings not published |

**Confirmed Tenants (via Public Sources):**
| Tenant | Property | Sector | Lease Details | Renewal |
|--------|----------|--------|---------------|---------|
| **Lumileds Malaysia Sdn Bhd** [T:174] | Bayan Lepas 1 & 2 | Electronics/Manufacturing (LED) | 15-year lease from Oct 2019, 3% annual escalation, first-year rent RM10.7m [T:175] | N/A |
| **Samsung SDS Malaysia Sdn Bhd** [T:176] | Shah Alam 2 | Logistics/3PL [T:177] | 2-year renewal in 2024, positive rental reversion [T:178] | ✅ 2024 |
| **Röhlig Malaysia Sdn Bhd** [T:179] | USJ (Block B) | Freight Forwarding/Logistics [T:180] | 3-year renewal in 2024 [T:181] | ✅ 2024 |
| **DSV Solutions (DC) Sdn Bhd** ⭐ [T:182] | Shah Alam 3 | Logistics/3PL (Danish MNC) [T:183] | 3-year renewal in early 2024 [T:184] | ✅ 2024 |
| **DHL Properties (Malaysia) Sdn Bhd** ⭐ [T:185] | Shah Alam 4 | Logistics/Courier (German MNC) [T:186] | Lease commenced Jan 2024 (post-AEI) [T:187] | New lease |
| **Lazada Express (Malaysia) Sdn Bhd** ⭐ [T:188] | Puchong | E-commerce Logistics [T:189] | — | — |
| **New tenant (undisclosed)** [T:190] | Shah Alam 5 | — | 3-year lease, commenced Sep 2025 [T:191] | Post-AEI |

**Tenant Identification Progress: 6 of 9 properties confirmed** (67% coverage) [T:192]
- ❓ ASA1: Current tenant unclear (Lazada/Ninja historically mentioned) [T:193]
- ❓ USJ Block A/C: Current tenants unclear [T:194]

**Industry Practice Note:** Unlike retail REITs that often disclose major anchor tenants, industrial REITs like Atrium typically treat tenant identities as proprietary information. Names emerge through annual reports, property portfolio disclosures, or industry sources (The Edge, EdgeProp, ChartNexus). **ASA5's new tenant remains the biggest mystery** - secured January 2025, commenced September 2025, but identity not publicly disclosed.

#### 2.7.3 Tenant Quality Indicators
| Proxy Indicator | Assessment |
|-----------------|------------|
| **Occupancy Rate** | 100% (Dec 2025) [T:195] | Indicates strong tenant demand |
| **Rental Collection** | No bad debt write-offs reported [T:196] | Implies reliable tenant payments |
| **Lease Renewals** | All 2025 expiries successfully renewed [T:197] | Tenant retention is strong |
| **Property Capex** | RM 34.44m invested [T:198] | Tenants willing to commit to upgraded premises |

**Assessment:** While specific tenant names and percentages are not disclosed, the 100% occupancy, successful lease renewals, and absence of bad debts suggest a high-quality tenant base with reliable credit profiles.

#### 2.7.3a 📈 2024 Lease Renewal Activity (Positive Momentum)

**Major Renewals in 2024:**
| Property | Tenant | Renewal | Terms | Outcome |
|----------|--------|---------|-------|---------|
| ASA2 | Samsung SDS Malaysia | 2-year renewal [T:199] | 2024 | ✅ **Positive rental reversion** |
| ASA3 | DSV Solutions (DC) | 3-year renewal [T:200] | Early 2024 | ✅ **Renewal secured** |
| USJ Block B | Röhlig Malaysia | 3-year renewal [T:201] | 2024 | ✅ **Positive rental reversion** |
| ASA4 | DHL Properties | New lease | Jan 2024 [T:202] | ✅ **Post-AEI tenant secured** |

**Key Insights:**
- **100% retention rate** for 2024 expiries [T:203] - all renewing tenants stayed
- **Positive rental reversions** on renewals (ASA2, USJ Block B) [T:204] - indicates market rent growth
- **AEI strategy working** - ASA4 secured DHL after enhancements completed in 2023 [T:205]
- **All confirmed tenants are logistics/e-commerce** [T:206] - pure-play sector focus aligns with Malaysia's industrial growth

**Tenant Sector Concentration:**
| Sector | Confirmed Tenants | Trend |
|--------|-------------------|-------|
| **Logistics/3PL** | DHL, DSV, Röhlig, Samsung SDS, Lazada | Growing e-commerce & supply chain demand |
| **Electronics Manufacturing** | Lumileds | Established industrial base |

**Assessment:** The 2024 renewal activity demonstrates strong tenant loyalty and market rent growth. All renewals included positive reversions, suggesting Atrium's properties are competitively positioned in the tight Malaysian logistics market.

#### 2.7.4 ⚠️ Key Tenant Deep Dive: Lumileds Malaysia Sdn Bhd

**CRITICAL FINDING:** Lumileds Malaysia is Atrium REIT's largest disclosed tenant (~21% of revenue) [T:207] with significant credit concerns at the parent company level.

**Company Profile:**
| Aspect | Details |
|--------|---------|
| **Parent Company** | Lumileds Holding B/V (Netherlands) [T:208] |
| **Origin** | Spun off from HP (1968) [T:209] → Philips (1999-2017) → Apollo PE (2017-2022) → Institutional lenders (2022-present) |
| **Global Position** | #3 in automotive LED lighting worldwide [T:210] |
| **Malaysia Operations** | 5,000+ employees [T:211]; Penang is a key global manufacturing hub |
| **Core Product** | LUXEON LED technology for automotive, mobile, and general lighting [T:212] |

**Atrium REIT Lease Arrangement:**
| Property | Acquisition | Annual Rent | Lease Term | Significance |
|----------|-------------|-------------|------------|--------------|
| Bayan Lepas 1 & 2 | Oct 2019 (sale-and-leaseback) [T:213] | **RM 10.7 million** [T:214] | 15 years [T:215] | ~21% of REIT's ~RM 50M revenue |
| | | (+3% annual escalation) [T:216] | (thru Oct 2034) [T:217] | Single largest tenant |

**⚠️ CREDIT RISK ALERT - Parent Company Financial Distress:**
| Date | Event | Risk Level |
|------|-------|------------|
| **Aug-Oct 2022** [T:218] | Parent filed Chapter 11 bankruptcy; debt reduced from $1.7B to $400M [T:219] | 🔴 High |
| **March 2025** [T:220] | S&P downgraded to **'SD' (Selective Default)** for deferring interest [T:221] | 🔴 **Critical** |
| **Jan 2025** [T:222] | Lost PIK interest option; must pay cash interest [T:223] | 🔴 High |
| **April 2026** [T:224] | Acquisition by San'an/Inari **blocked by US CFIUS** [T:225] | 🔴 Uncertainty |

**Current S&P Rating:** **SD (Selective Default)** as of March 3, 2025 [T:226]

**Financial Health (Parent Company):**
| Metric | Status |
|--------|--------|
| Credit Rating | SD (Selective Default) - deferred interest payment [T:227] |
| Free Operating Cash Flow | Negative [T:228] |
| Liquidity | Constrained [T:229] |
| LED Segment EBITDA (2024) | Improved to +$9M (vs -$47M in 2023) [T:230] |

**Tenant Risk Assessment:**
| Factor | Rating | Assessment |
|--------|--------|------------|
| **Business Quality** | ★★★☆☆ [T:231] | Global top-3 position, strong IP, 1 in 3 cars worldwide use Lumileds [T:232] |
| **Financial Health** | ★★☆☆☆ [T:233] | Parent in selective default; bankruptcy history |
| **Tenant Stability** | ★★★☆☆ [T:234] | 15-year lease shows commitment; ops appear stable in Malaysia |
| **Industry Outlook** | ★★★★☆ [T:235] | Growing automotive LED market (EVs, ADAS, smart lighting) [T:236] |
| **Counterparty Risk** | ★★☆☆☆ [T:237] | Significant credit risk at parent level |

**Overall Tenant Risk Rating: MODERATE-HIGH (2.8/5)** [T:238]

**Positive Mitigating Factors:**
- ✅ 15-year locked-in lease through 2034 with 3% annual escalations [T:239]
- ✅ Penang facility is strategically important to global automotive supply chain [T:240]
- ✅ Malaysia subsidiary appears operationally stable (5,000+ employees) [T:241]
- ✅ 100% renewable electricity since 2022 [T:242] shows operational commitment
- ✅ LED automotive market growing (4.7-9% CAGR) driven by EV adoption [T:243]

**Negative Risk Factors:**
- 🔴 Parent company in **selective default** (S&P rating 'SD') [T:244]
- 🔴 **~21% revenue concentration** [T:245] - single tenant loss would be severe
- 🔴 Recent Chapter 11 bankruptcy (2022) [T:246] and ongoing financial restructuring
- 🔴 Failed acquisition (April 2026) [T:247] creates ownership uncertainty
- 🔴 Negative free cash flow and liquidity constraints at parent level [T:248]

**Key Risk Summary for Atrium REIT Investors:**
> Lumileds represents a **"good company, bad balance sheet"** scenario. The Malaysia operations are strategically important with a long-term lease commitment, but the parent company's financial distress (S&P 'SD' rating) introduces material counterparty risk. The 15-year lease provides contractual protection, but investors should monitor any restructuring actions that might affect subsidiary operations. The concentration risk (~21% of revenue) amplifies the impact of any Lumileds-related issues.

**Recommendation:** Monitor Lumileds' parent company restructuring developments, any announcements regarding the Malaysia subsidiary, and Atrium REIT's tenant concentration disclosures in future filings.

### 2.8 Lease Expiry Profile (WALE)

#### 2.8.1 Weighted Average Lease Expiry (WALE)
| Metric | Status | Value |
|--------|--------|-------|
| **WALE (Years)** | **Not Disclosed** | Not reported in public filings |
| **Average Lease Term** | Estimated | Likely 3-5 years (typical for industrial) |
| **2025 Lease Renewals** | 100% Success Rate | All expiring leases renewed |

#### 2.8.2 Lease Expiry Schedule (Estimated Pattern)
| Expiry Period | Estimated % of Gross Rental Income | Risk Level |
|---------------|-----------------------------------|------------|
| **2026** | Not disclosed | Unknown |
| **2027** | Not disclosed | Unknown |
| **2028** | Not disclosed | Unknown |
| **2029+** | Not disclosed | Unknown |

**Note:** Atrium REIT does not publicly disclose its lease expiry ladder or WALE. This is less common than for office/retail REITs where tenant rollover risk is more closely monitored. Industrial REITs typically have longer lease terms with built-in renewals.

#### 2.8.3 Lease Structure Characteristics
| Feature | Typical Terms |
|---------|-------------|
| **Lease Duration** | 3-7 years (estimated from industrial norms) |
| **Rental Escalation** | Built-in annual increases (typically 2-3%) |
| **Renewal Options** | Tenant renewal options common |
| **Lease Type** | Triple-net or equivalent (tenant pays maintenance, insurance, taxes) |

**Assessment:** The lack of disclosed WALE and lease expiry profile is a data gap. However, the 100% occupancy and successful 2025 renewals suggest manageable rollover risk. The built-in rental escalations provide organic income growth.

---

## 3. Debt Sustainability Analysis

### 3.1 Capital Structure (Dec 31, 2025)
| Component | Amount (RM million) | Percentage |
|-----------|---------------------|------------|
| **Total Assets** | 723.24 [T:249] | 100.0% |
| **Total Borrowings** | 314.79 [T:250] | 43.5% [T:251] |
| **Non-current Borrowings** | 262.87 [T:252] | 36.3% [T:253] |
| **Current Borrowings** | 51.92 [T:254] | 7.2% [T:255] |
| **Total Equity/NAV** | 408.45 [T:256] | 56.5% [T:257] |

### 3.2 Gearing Ratio Analysis
| Metric | FY 2025 | Regulatory Limit | Headroom |
|--------|---------|------------------|----------|
| **Gearing Ratio** | **43.5%** [T:258] | 60.0% (SC Malaysia REIT limit) [T:259] | 16.5% [T:260] |

**Assessment:** The gearing ratio of 43.5% [T:261] provides a comfortable buffer of 16.5 percentage points [T:262] below the regulatory ceiling. This conservative leverage position provides flexibility for future acquisitions or refinancing needs.

### 3.3 Interest Coverage Ratio
| Metric | Value | Assessment |
|--------|-------|------------|
| **Interest Coverage** | **~2.09x** [T:263] | Minimum threshold is typically 2.0x |
| **Finance Costs (FY 2025)** | ~RM 6.39 million [T:264] | Interest expense on total borrowings |

**Assessment:** The interest coverage ratio of ~2.09x [T:265] is adequate, indicating earnings cover interest expenses approximately 2 times. However, this is at the lower end of comfort (typically 3.0x+ is preferred) and warrants monitoring, especially if interest rates rise further.

### 3.4 Detailed Debt Maturity Profile

#### 3.4.1 Current vs Non-Current Breakdown (Dec 31, 2025)
| Category | Amount (RM million) | Percentage |
|----------|---------------------|------------|
| **Current Borrowings (Due within 12 months)** | 51.92 [T:266] | 16.5% [T:267] |
| - Bank Overdraft | ~9.3 [T:268] | Demand/On call |
| - Short-term Revolving Credit | ~39.6 [T:269] | Due Feb 2026 (renewed annually) [T:270] |
| - Current portion of Term Loans | ~3.0 [T:271] | Principal repayments |
| **Non-current Borrowings (Long-term)** | 262.87 [T:272] | 83.5% [T:273] |
| - Term Loans (non-current portion) | ~61.0 [T:274] | 1-5 years [T:275] |
| - Medium Term Notes (MTN) | ~233.7 [T:276] | Various tenures |
| **Total Borrowings** | **314.79** [T:277] | **100%** |

#### 3.4.2 Debt Maturity Schedule (Historical Pattern from 2024 Annual Report)
| Due Period | Bank Overdraft | Term Loans | MTN | Revolving Credit | **Total** |
|------------|----------------|------------|-----|------------------|-----------|
| **On Demand / Within 1 Year** | RM 9.3m [T:278] | RM 2.4m [T:279] | RM 8.6m [T:280] | RM 41.3m [T:281] | **RM 61.6m** [T:282] |
| **1-5 Years** | - | RM 61.8m [T:283] | RM 154.1m [T:284] | - | **RM 215.9m** [T:285] |
| **Over 5 Years** | - | - | RM 71.0m [T:286] | - | **RM 71.0m** [T:287] |

#### 3.4.3 Key 2025 Debt Events
| Event | Date | Details |
|-------|------|---------|
| **MTN Tranche 5A Issuance** | September 8, 2025 [T:288] | RM 18.84 million unrated MTN, fully subscribed by Public Bank Berhad [T:289] |
| **MTN Tranche 5 Terms** | 2025 | 7-year tenure with single lump-sum redemption at maturity (due 2032) [T:290] |
| **Revolving Credit Renewal** | February 2026 [T:291] | RM 39.6 million facility due for renewal (historically renewed annually) [T:292] |

#### 3.4.4 Debt Maturity Concentration Assessment
| Risk Period | Amount Due | % of Total Debt | Risk Level |
|-------------|------------|-----------------|------------|
| 2026 | ~RM 52m [T:293] | 16.5% [T:294] | 🟢 Low - revolving credit historically renewed |
| 2027-2029 | ~RM 216m [T:295] | 68.6% [T:296] | 🟡 Medium - staggered maturities expected |
| 2030+ | ~RM 71m [T:297] | 22.6% [T:298] | 🟢 Low - MTN Tranche 5 due 2032 [T:299] |

**Assessment:** The debt maturity profile is well-laddered with no significant refinancing cliffs in the near term. The RM 39.6 million [T:300] revolving credit is due February 2026 but has been renewed annually. The MTN programme provides significant refinancing flexibility (RM 999 million authorized [T:301], ~RM 234 million drawn as of 2025 [T:302]).

### 3.5 Debt Financing & Interest Rate Profile

#### 3.5.1 Fixed vs Floating Rate Debt Composition
| Debt Type | Amount (RM million) | Rate Type | Est. % of Total |
|-----------|---------------------|-----------|-----------------|
| **Bank Overdraft** | ~9.3 [T:303] | Floating | ~3% [T:304] |
| **Term Loans** | ~64.1 [T:305] | Floating | ~20% [T:306] |
| **Medium Term Notes (MTN)** | ~233.7 [T:307] | Floating | ~74% [T:308] |
| **Short-term Revolving Credit** | ~39.6 [T:309] | Floating | ~13% [T:310] |
| **Total Floating Rate Exposure** | **~346.7** [T:311] | - | **~93%** [T:312] |

**⚠️ Important:** Atrium REIT's debt is **predominantly floating-rate** (~93% estimated) [T:313], making it highly sensitive to changes in Bank Negara Malaysia's Overnight Policy Rate (OPR) [T:314].

#### 3.5.2 Interest Rate Risk Management
| Aspect | Status | Details |
|--------|--------|---------|
| **Interest Rate Swaps** | ❌ **Not Used** [T:315] | No hedging instruments in place |
| **Interest Rate Caps** | ❌ **Not Used** [T:316] | No derivative protection |
| **Management Approach** | Active Monitoring [T:317] | Tracks BNM Monetary Policy Committee updates [T:318] |
| **Mitigation Strategy** | Capital Structure [T:319] | Manages through conservative gearing rather than derivatives |

#### 3.5.3 Interest Rate Sensitivity Analysis
| Scenario | OPR Change | Estimated Annual Impact on Finance Costs |
|----------|------------|------------------------------------------|
| Base Case | Current levels | ~RM 6.39 million (FY 2025) [T:320] |
| +25 bps | +0.25% [T:321] | +~RM 0.87 million [T:322] |
| +50 bps | +0.50% [T:323] | +~RM 1.73 million [T:324] |
| -25 bps | -0.25% [T:325] | -~RM 0.87 million [T:326] |

*Note: Sensitivity calculated based on ~93% floating rate exposure on RM 314.79 million borrowings* [T:327]

#### 3.5.4 Weighted Average Cost of Debt
| Metric | Estimated Value | Data Source |
|--------|-----------------|------------- |
| **Estimated WACD** | ~3.5-4.0% [T:328] | Derived from finance costs / average borrowings |
| **Finance Costs (FY 2025)** | RM 6.39 million [T:329] | Q4 2025 financial statements [T:330] |
| **Currency Exposure** | 100% MYR [T:331] | No foreign currency debt |

**Assessment:** Atrium REIT has significant interest rate exposure due to its ~93% floating-rate debt composition [T:332]. The absence of hedging instruments is a key risk factor if Bank Negara Malaysia raises rates. The estimated WACD of 3.5-4.0% [T:333] is competitive but could rise with OPR increases.

### 3.6 Financing Facilities
| Facility | Status | Amount Drawn | Authorized Limit |
|----------|--------|--------------|------------------|
| **Medium Term Notes (MTN) Programme** | Active | ~RM 233.7m | Up to RM 999 million (unrated) |
| **Term Loans** | Active | ~RM 64.1m | Various bank facilities |
| **Revolving Credit** | Active (renewable) | ~RM 39.6m | Annual renewal pattern |
| **Bank Overdraft** | Active | ~RM 9.3m | Working capital facility |

### 3.7 Debt Sustainability Verdict
| Factor | Rating | Rationale |
|--------|--------|-----------|
| **Gearing Level** | ✅ **Sustainable** | 43.5% is conservative vs 60% limit |
| **Interest Coverage** | ⚠️ **Monitor** | 2.09x is adequate but not strong |
| **Maturity Profile** | ✅ **Sustainable** | 68.6% due 2027-2029, no near-term cliffs |
| **Refinancing Risk** | ✅ **Low** | RM 999 million MTN programme provides flexibility |
| **Interest Rate Risk** | 🔴 **High Exposure** | ~93% floating-rate, no hedging instruments |

---

## 4. Dividend Sustainability Analysis

### 4.1 2025 Income Distributions
| Distribution | DPU (sen/unit) | Ex-Date | Payment Date | Amount (RM million) |
|--------------|---------------|---------|--------------|---------------------|
| First Interim | 0.90 [T:334] | Feb 2025 [T:335] | Feb 26, 2025 [T:336] | ~1.35 [T:337] |
| Second Interim | 2.10 [T:338] | May 2025 [T:339] | May 30, 2025 [T:340] | ~3.15 [T:341] |
| Third Interim | 2.20 [T:342] | Aug 2025 [T:343] | Aug 29, 2025 [T:344] | ~3.30 [T:345] |
| Fourth Interim | 2.40 [T:346] | Nov 2025 [T:347] | Nov 28, 2025 [T:348] | ~3.60 [T:349] |
| Fifth/Final | 1.70 [T:350] | Dec 2025 [T:351] | Dec 31, 2025 [T:352] | ~2.55 [T:353] |
| **Total FY 2025** | **9.30 sen** [T:354] | - | - | **~13.95** [T:355] |

### 4.2 Historical DPU Trend
| Year | Annual DPU (sen) | YoY Change | Notes |
|------|------------------|------------|-------|
| 2021 | ~10.0 [T:356] | - | Peak year |
| 2022 | ~9.0 [T:357] | -10% [T:358] | Post-pandemic normalization |
| 2023 | ~7.5 [T:359] | -16.7% [T:360] | DPU decline period |
| 2024 | ~7.6 [T:361] | +1.3% [T:362] | Stabilization began |
| **2025** | **9.30** [T:363] | **+22.4%** [T:364] | **Strong recovery** |

**Assessment:** The 22.4% year-over-year DPU growth in 2025 [T:365] demonstrates significant improvement, driven by ASA5 tenancy commencement and full portfolio occupancy.

### 4.3 Distribution Coverage Analysis
| Metric | FY 2025 Value | Assessment |
|--------|---------------|------------|
| **Realised Income** | RM 46.61 million [T:366] | Income available for distribution |
| **Total Distributions** | ~RM 13.95 million [T:367] | Based on 9.30 sen DPU |
| **Distribution Payout Ratio** | ~29.9% [T:368] | Conservative payout |
| **REIT Requirement** | Minimum 90% of taxable income [T:369] | Compliant |

**Note:** Malaysian REITs must distribute at least 90% of taxable income [T:370] to maintain tax-exempt status at the trust level. Atrium REIT's payout policy meets this requirement while maintaining a sustainable distribution level.

### 4.4 Dividend Yield Analysis

#### Current Market Data (April 2026)
| Metric | Value | Source |
|--------|-------|--------|
| **Current Share Price** | **RM 1.280** [T:371] | KLSE Screener [T:372] |
| **NAV per Unit** | RM 1.3874 [T:373] | FY 2025 Annual Report |
| **2025 DPU** | 9.30 sen [T:374] | Full year distribution |
| **Market Dividend Yield** | **7.27%** [T:375] | 9.30 sen ÷ RM 1.280 |
| **Price-to-Book (P/B)** | **0.92x** [T:376] | Trading at 8% discount to NAV [T:377] |
| **Market Cap** | RM 340 million [T:378] | Small-cap classification |

#### Understanding Yield Metrics

| Yield Type | Calculation | Atrium REIT | What It Means |
|------------|-------------|-------------|---------------|
| **Market Yield** | DPU ÷ Current Share Price | **7.27%** [T:379] | ✅ Actual investor return |
| **NAV-Based Yield** | DPU ÷ NAV per unit | **6.7%** [T:380] | Operational efficiency metric |

**Why the difference?**
- Atrium REIT trades at **RM 1.280** [T:381] vs NAV of **RM 1.3874** [T:382]
- This is an **8% discount to NAV** (P/B = 0.92x) [T:383]
- The discount provides **yield enhancement**: 7.27% market yield vs 6.7% NAV yield [T:384]

#### Comparison Context (April 2026)
| REIT | Current Price | NAV | P/B | Market Yield | Status |
|------|---------------|-----|-----|--------------|--------|
| **Atrium REIT** | RM 1.280 [T:385] | RM 1.3874 [T:386] | **0.92x** [T:387] | **7.27%** [T:388] | Discount |
| **Axis REIT** | RM 2.050 [T:389] | RM 1.6907 [T:390] | **1.21x** [T:391] | **5.15%** [T:392] | Premium |
| **Difference** | - | - | - | **+2.12pp** [T:393] | Atrium advantage |

#### Key Insights

**Atrium REIT's 7.27% Market Yield:**
- ✅ Higher than Axis REIT's 5.15% (by 2.12 percentage points) [T:394]
- ⚠️ But trades at **discount** (0.92x P/B) [T:395] vs Axis at **premium** (1.21x P/B) [T:396]
- ⚠️ Discount reflects market's risk pricing: liquidity, concentration, interest rate exposure [T:397]

**Yield Premium is Compensation for Risk:**
The ~2.1pp yield advantage is **not free money** [T:398] - it compensates for:
- Liquidity risk (RM 340M vs RM 4.15B market cap) [T:399]
- Concentration risk (9 vs 69 properties, Lumileds ~21% income) [T:400]
- Interest rate risk (~93% floating vs 65% fixed) [T:401]
- Transparency gaps (no WALE, tenant % disclosed) [T:402]

**⚠️ For Investment Decisions:** Always use **market price-based yield** (7.27% at current RM 1.280) [T:403], not NAV yield. Verify latest price on [KLSE Screener](https://www.klsescreener.com/v2/stocks/5130) or [Bursa Malaysia](https://www.bursamalaysia.com).

### 4.5 Dividend Sustainability Factors
| Factor | Status | Impact on DPU |
|--------|--------|---------------|
| **100% Occupancy** | ✅ Positive | Stable rental income base |
| **ASA5 Contribution** | ✅ Positive | Full year contribution from 2026 |
| **Long-term Leases** | ✅ Positive | Predictable cash flows |
| **Conservative Gearing** | ✅ Positive | Financial flexibility maintained |
| **Interest Rate Environment** | ⚠️ Watch | May pressure margins if rates rise |

### 4.6 Dividend Sustainability Verdict
| Factor | Rating | Rationale |
|--------|--------|-----------|
| **Payout Ratio** | ✅ **Sustainable** | Conservative at ~29.9% of realised income |
| **Income Stability** | ✅ **Sustainable** | 100% occupancy, long-term leases |
| **Growth Trajectory** | ✅ **Improving** | 22.4% YoY DPU growth |
| **REIT Compliance** | ✅ **Compliant** | Meets 90% distribution requirement |

---

## 5. Financial Performance Summary (FY 2025)

### 5.1 Income Statement Highlights
| Metric | FY 2025 (RM million) | Change vs FY 2024 |
|--------|---------------------|-------------------|
| **Gross Revenue** | 51.05 [T:404] | +Stable |
| **Net Property Income (NPI)** | 45.98 [T:405] | Strong margin ~90% [T:406] |
| **Realised Income** | 46.61 [T:407] | Available for distribution |
| **Finance Costs** | ~6.39 [T:408] | Interest expense |
| **Q4 Net Loss** | (4.91) [T:409] | Unrealized items impact |

**Note:** The Q4 2025 net loss of RM 4.91 million [T:410] was due to unrealized items (likely property revaluation or non-cash adjustments) and does not affect realised income available for distribution.

### 5.2 Quarterly Revenue Trend (2025)
| Quarter | Revenue (RM million) | Trend |
|---------|-------------------|-------|
| Q1 2025 | ~11.0 [T:411] | Baseline |
| Q2 2025 | ~11.5 [T:412] | Growing |
| Q3 2025 | 11.49 [T:413] | Pre-ASA5 commencement |
| Q4 2025 | 12.41 [T:414] | ASA5 contribution |

**Assessment:** Revenue shows steady growth trajectory, with Q4 benefiting from ASA5's full tenancy commencement in September 2025 [T:415].

---

## 6. Industry Context & Benchmark Analysis

### 6.1 Malaysia Industrial Property Market Outlook (2024-2026)

The Malaysian industrial property sector has emerged as the **standout performer** in the country's real estate market, significantly outperforming office and retail segments.

| Metric | Current Data | Trend |
|--------|--------------|-------|
| **Prime Logistics Vacancy** | 2.0% (Q2 2025) [T:416] | ⬇️ Down from 4.8% (Q4 2024) [T:417] |
| **Market Tightness** | Very tight supply [T:418] | Driving rental growth |
| **FDI Inflows** | Strong in semiconductors, E&E, automotive [T:419] | Supporting demand |
| **Flight to Quality** | Green assets occupy 30% faster [T:420] | ESG becoming critical |

**Key Market Drivers:**
- **New Industrial Master Plan 2030** [T:421] - Government policy supporting manufacturing growth
- **Johor-Singapore Special Economic Zone (JS-SEZ)** [T:422] - Enhancing cross-border logistics
- **Port Klang Upgrades** [T:423] - Major logistics hub expansion
- **East Coast Rail Link (ECRL)** [T:424] - Improving connectivity
- **Data Center Boom** [T:425] - Hyperscale facilities driving land values in Selangor/Johor

**Assessment:** Atrium REIT operates in a favorable macro environment with tight supply conditions and strong tenant demand. The Klang Valley concentration (78% of portfolio) is well-positioned given Port Klang's role as Malaysia's premier logistics hub.

### 6.2 Asia Pacific Logistics REIT Sector Benchmarks

Comparison of Atrium REIT against regional industrial/logistics REIT standards:

| Metric | Atrium REIT (5130) | Industry Benchmark | Assessment |
|--------|-------------------|-------------------|------------|
| **Portfolio Size** | 9 properties [T:426] | 50-180 properties (large-cap) [T:427] | Small-cap; concentration risk |
| **AUM** | RM 723m (~USD 160m) [T:428] | S$13.3b (MLT reference) [T:429] | Micro-cap in regional context |
| **Occupancy Rate** | **100%** [T:430] | 94-96% (APAC avg) [T:431] | ✅ **Above benchmark** |
| **Gearing Ratio** | **43.5%** [T:432] | 30-43% (typical range) [T:433] | ⚠️ **At higher end** |
| **Interest Coverage** | **~2.09x** [T:434] | 2.5-4.0x (healthy range) [T:435] | 🔴 **Below benchmark** |
| **WALE** | Not disclosed [T:436] | 3.0-5.0 years (typical) [T:437] | Unknown - data gap |
| **DPU Growth (2025)** | **+22.4%** [T:438] | 10-15% (select REITs) [T:439] | ✅ **Strong outperformance** |

**Reference: Mapletree Logistics Trust (MLT)** [T:440] - Leading Asia Pacific logistics REIT:
- 180 properties across 9 markets [T:441]
- S$13.3 billion AUM [T:442]
- 96.2% occupancy (FY24/25) [T:443]
- Rental reversion: +2.1% average (up to +27.9% in Australia) [T:444]
- 56% of portfolio with green certification [T:445]

**Source:** [Mapletree Logistics Trust Independent Market Research Report 2024-25](https://investor.mapletreelogisticstrust.com/misc/independent-market-research-report24-25.pdf)

### 6.3 Small-Cap vs Large-Cap Industrial REIT Comparison

| Factor | Large-Cap REITs (e.g., MLT, Ascendas) | Small-Cap REITs (e.g., Atrium) |
|--------|--------------------------------------|-------------------------------|
| **Funding Costs** | Lower (30-40 bps advantage) | Higher due to limited credit ratings |
| **Liquidity** | High; institutional-friendly | Low to moderate; "liquidity discount" |
| **Valuation** | Trades at or near NAV | Often trades at NAV discount |
| **Diversification** | Geographic & tenant diversification | High concentration risk |
| **Analyst Coverage** | Extensive | Limited |
| **Yield** | Lower (liquidity premium) | Higher (illiquidity compensation) |
| **Growth Flexibility** | Scale advantages | Nimble; "needle-moving" acquisitions |
| **M&A Dynamic** | Acquirers | Potential acquisition targets |

**Atrium REIT Positioning:**
- Micro-cap industrial pure-play in Malaysian context
- Benefits from "flight to quality" trend but lacks ESG certification disclosure
- Concentration risk offset by 100% occupancy and MNC tenant base
- Higher yield potential but lower liquidity than large-cap peers

### 6.4 Key Industry Trends Affecting Atrium REIT

| Trend | Impact on Atrium REIT | Assessment |
|-------|------------------------|------------|
| **E-commerce Growth** | Positive - demand for fulfillment centers | ✅ Favorable |
| **Supply Chain Diversification ("China+n")** | Positive - Malaysia benefiting from manufacturing shifts | ✅ Favorable |
| **3PL Provider Expansion** | Positive - MNC logistics demand | ✅ Favorable |
| **ESG/Green Certification** | Risk - no disclosed green certification | ⚠️ Monitor |
| **Data Center Demand** | Indirect positive - land value appreciation | ✅ Favorable |
| **Interest Rate Cycle** | Risk - ~93% floating-rate debt exposure | 🔴 High Risk |
| **JS-SEZ Development** | Limited direct impact - focused on Klang Valley | ⚪ Neutral |
| **Flight to Quality** | Risk/Opportunity - older assets may face pricing pressure | ⚠️ Mixed |

### 6.5 Industry Benchmark Verdict

| Category | Atrium REIT vs Industry | Key Insight |
|----------|------------------------|-------------|
| **Occupancy** | ✅ **Outperforming** | 100% vs 94-96% benchmark |
| **Gearing** | ⚠️ **Higher than ideal** | 43.5% at upper end of 30-43% range |
| **Interest Coverage** | 🔴 **Below standard** | 2.09x vs 2.5-4.0x healthy range |
| **DPU Growth** | ✅ **Outperforming** | +22.4% vs 10-15% sector average |
| **Scale** | 🔴 **Micro-cap** | RM 723m vs billions for regional peers |
| **Transparency** | ⚠️ **Gaps** | WALE, tenant %, lease expiry not disclosed |

**Strategic Implications:**
Atrium REIT demonstrates strong operational metrics (occupancy, DPU growth) but faces structural challenges common to small-cap REITs: higher gearing, lower interest coverage, limited transparency, and concentration risk. The favorable Malaysian industrial market backdrop provides tailwinds, but the ~93% floating-rate debt exposure without hedging is a significant outlier risk compared to better-capitalized peers.

---

## 7. Malaysian Peer Comparison: Atrium REIT vs Axis REIT

### 7.1 Head-to-Head Comparison Table (FY 2025 Actuals)

| Metric | Atrium REIT (5130) | Axis REIT (5106) | Winner | Delta |
|--------|-------------------|------------------|--------|-------|
| **Stock Code** | 5130.KL [T:446] | 5106.KL [T:447] | - | Both Bursa Malaysia Main Market |
| **Portfolio Size** | 9 properties [T:448] | 69 properties [T:449] | Axis | 7.7x more properties |
| **Total Assets (AUM)** | RM 723 million [T:450] | RM 5.36 billion [T:451] | Axis | 7.4x larger |
| **Property Value** | RM 685 million [T:452] | ~RM 5.0 billion [T:453] | Axis | Based on disclosed data |
| **Geographic Coverage** | Klang Valley (78%) + Penang (22%) [T:454] | Nationwide (6 states) [T:455] | Axis | Better diversification |
| **Occupancy Rate** | **100%** (Q4 2025) [T:456] | **94%** (Dec 2025) [T:457] | Atrium | 6pp higher |
| **WALE** | Not disclosed [T:458] | **4.4 years** [T:459] | Axis | Known vs unknown |
| **Gearing Ratio** | **43.5%** [T:460] | **~33%** [T:461] | Axis | 10.5pp lower leverage |
| **Interest Coverage** | **~2.09x** [T:462] | **3.8-4.0x** [T:463] | Axis | +1.7x better coverage |
| **Fixed-Rate Debt** | **~7%** (est. from ~93% floating) [T:464] | **65%** [T:465] | Axis | 9x more fixed |
| **Floating-Rate Exposure** | **~93%** [T:466] | **~35%** [T:467] | Axis | 58pp lower risk |
| **Hedging Strategy** | None [T:468] | Active (Islamic profit rate swaps) [T:469] | Axis | Risk managed |
| **Unencumbered Assets** | Not disclosed | 38% (23 of 69 assets) [T:470] | Axis | Known flexibility |
| **2025 Revenue** | RM 51.05M [T:471] | **RM 364.2M** [T:472] | Axis | 7.1x larger |
| **2025 NPI** | RM 45.98M [T:473] | **RM 316.2M** [T:474] | Axis | 6.9x larger |
| **2025 DPU** | 9.30 sen [T:475] | **10.55 sen** [T:476] | Axis | +13.4% higher |
| **DPU Growth (2025)** | +22.4% [T:477] | +13.8% [T:478] | Atrium | +8.6pp higher growth |
| **NAV-Based Yield** | ~6.7% [T:479] | ~6.2% [T:480] | Atrium | +0.5pp |
| **Market Yield (Actual)** | **7.27%** [T:481] | **5.15%** [T:482] | Atrium | **+2.12pp higher** |
| **Trading Status** | 8% discount (P/B 0.92x) [T:483] | 21% premium (P/B 1.21x) [T:484] | - | Opposite valuations |

*Source: KLSE Screener, April 2026. Atrium RM 1.280 [T:485], Axis RM 2.050 [T:486]
| **Payout Ratio** | ~30% [T:487] | ~75-76% [T:488] | Atrium | More conservative |
| **NAV per Unit** | RM 1.3874 [T:489] | RM 1.64 [T:490] | Axis | +18.2% higher |
| **Tenant Count** | Not disclosed | **182 tenants** [T:491] | Axis | Diversification |
| **Top 10 Tenant %** | Not disclosed | 46.7% [T:492] | Axis | Disclosure advantage |
| **Analyst Coverage** | Limited/None | Moderate [T:493] | Axis | Better research support |
| **Shariah Compliance** | No | **Yes** [T:494] | Axis | Islamic investor access |
| **Market Cap** | ~RM 600m [T:495] | ~RM 3.5b [T:496] | Axis | 5.8x larger |
| **Liquidity** | Lower | Higher [T:497] | Axis | Easier to trade |
| **2025 Acquisitions** | 0 (ASA5 AEI only) | **3 completed** (RM 164.6M+) [T:498] | Axis | Active growth |
| **2025 Disposals** | 0 | **2 completed** (The Annex) [T:499] | Axis | Portfolio optimization |
| **Acquisition Pipeline** | None | Senai forward (2027) [T:500] | Axis | Future income |
| **MTN Programme** | RM 999m authorized [T:501] | RM 3.0b Islamic MTN [T:502] | Comparable | Both have capacity |
| **ESG/Green Certification** | Not disclosed | Disclosed initiatives [T:503] | Axis | Better ESG |
| **Listing History** | ~10 years [T:504] | **20 years** (2005-2025) [T:505] | Axis | Track record |
| **Management Scale** | Smaller team | Larger team [T:506] | Axis | Resources |
| **Asset Age Profile** | 44% from 2007 IPO [T:507]; Bayan Lepas 1 = 54 years [T:508] | More diversified; active recycling [T:509] | Axis | Better age diversification |
| **AEI/Capex Activity** | RM 34.4M (2025) [T:510], RM 32.4M (ASA5) [T:511] | ~RM 13M+ (2025) [T:512] | Atrium | Higher % of AUM spent on AEIs |

### 7.2 Scale & Diversification Analysis

#### Size Impact on Operations
| Factor | Atrium REIT (Small) | Axis REIT (Large) | Implication |
|--------|---------------------|-------------------|-------------|
| **Acquisition Impact** | High - one property moves needle | Low - need multiple for impact | Atrium: Higher growth potential per deal |
| **Tenant Loss Impact** | Severe - 11% of portfolio per tenant | Minor - 1.4% per tenant | Atrium: Higher concentration risk |
| **Operating Leverage** | Limited economies of scale | Cost efficiencies | Axis: Better margin potential |
| **Bargaining Power** | Limited with tenants/suppliers | Stronger negotiating position | Axis: Better lease terms |
| **Management Fees** | Higher % of revenue | Lower % (scale) | Atrium: Higher cost drag |

### 7.3 Financial Health Comparison

#### Debt & Coverage Metrics
```
Metric                    Atrium REIT    Axis REIT      Industry Pref  Winner
────────────────────────────────────────────────────────────────────────────
Gearing Ratio             43.5%          ~33%           < 40%          ✅ Axis
Interest Coverage         ~2.09x         3.8-4.0x       > 2.5x         ✅ Axis
Fixed-Rate Debt           ~7%            65%            > 50%          ✅ Axis
Floating Rate Exposure    ~93%           ~35%           < 50%          ✅ Axis
Debt Maturity (Short-term)~16.5%          ~35%           < 40%          ✅ Axis
Hedging Strategy          None           Active swaps   Yes            ✅ Axis
Unencumbered Assets       Unknown        38%            > 30%          ✅ Axis
Sukuk/Islamic Finance     None           RM 300M issued Shariah OK      ✅ Axis
```

**Verdict:** Axis REIT has **materially stronger debt metrics** across all dimensions:
- **Gearing:** 10.5pp lower (33% vs 43.5%)
- **Coverage:** 81% better (3.8-4.0x vs 2.09x)
- **Rate Protection:** 9x more fixed-rate debt (65% vs ~7%)
- **Risk Management:** Active hedging vs none

Atrium REIT's ~93% floating-rate exposure without hedging is a **significant outlier risk** in a rising rate environment.

#### Income & Distribution Comparison
```
Metric                    Atrium REIT    Axis REIT      Assessment
─────────────────────────────────────────────────────────────────────────
Revenue (FY2025)          RM 51.1M       RM 364.2M      Axis 7.1x larger
NPI (FY2025)              RM 46.0M       RM 316.2M      Axis 6.9x larger
Occupancy Rate            100%           94%            Atrium 6pp higher
WALE (Income Visibility)  Unknown        4.4 years      Axis known stability
Tenant Count              Not disclosed  182            Axis diversification
Top 10 Tenant %           Unknown        46.7%          Axis disclosure
2025 DPU                  9.30 sen       10.55 sen      Axis +13.4% higher
DPU Growth (YoY)          +22.4%         +13.8%         Atrium momentum
NAV-Based Yield           ~6.7%          ~6.2%          Atrium +0.5pp
Market Yield (Actual)     **7.27%**      **5.15%**      Atrium **+2.12pp**
Current Price (April)     RM 1.280       RM 2.050       - RM 0.77 diff
P/B Ratio                 0.92x (disc)   1.21x (prem)   Opposite valuations
Payout Ratio              ~30%           ~75-76%        Atrium conservative
```

**Verdict:** 
- **Axis REIT delivers higher absolute DPU** (10.55 sen vs 9.30 sen) with better disclosure
- **Atrium REIT shows stronger momentum** (+22.4% vs +13.8% growth) and higher yield
- **Axis REIT offers income stability** with 4.4-year WALE and 182-tenant diversification
- **Atrium REIT has payout headroom** (~30% vs ~75%) for future increases

### 7.4 Strategic Positioning Comparison

#### Atrium REIT Strategy
- **Focus:** Pure-play Klang Valley/Penang industrial concentration
- **Approach:** Asset enhancement (AEI) over acquisitions
- **Risk Profile:** Higher concentration, higher gearing, floating-rate exposure
- **Target Investors:** Yield-focused, risk-tolerant investors seeking higher returns
- **Catalysts:** ASA5 full-year contribution, potential portfolio expansion

#### Axis REIT Strategy  
- **Focus:** Diversified nationwide industrial portfolio (6 states, 69 properties)
- **Approach:** Active capital recycling - 3 acquisitions + 2 disposals in 2025
- **Risk Profile:** Lower concentration (33% gearing, 65% fixed-rate, active hedging)
- **Target Investors:** Institutional, Islamic, stability-focused investors
- **Catalysts:** Senai forward purchase (2027), RM 3.0B MTN capacity, 20-year track record

### 7.5 Risk-Adjusted Comparison

#### Risk Factors by REIT
| Risk Category | Atrium REIT | Axis REIT | Mitigation Leader |
|---------------|-------------|-----------|-------------------|
| **Concentration Risk** | 🔴 High (9 properties) | 🟢 Low (69 properties, 182 tenants) | Axis |
| **Single Tenant Credit Risk** | 🔴 **Lumileds = ~21% income; parent in S&P 'SD' default** | 🟢 Top 10 = 46.7%, diversified | **Axis** |
| **Tenant Rollover Risk** | 🔴 Unknown (no WALE disclosed) | 🟢 Low (4.4yr WALE, 73% renewal) | Axis |
| **Interest Rate Risk** | 🔴 **Critical (~93% floating, NO hedging)** | 🟢 **Managed (65% fixed, active hedging)** | **Axis** |
| **Liquidity Risk** | 🟡 Medium (low volume) | 🟢 Low (mid-cap volume) | Axis |
| **Gearing Risk** | 🟡 Higher (43.5%) | 🟢 Low (~33%) | Axis |
| **Market Cap Risk** | 🔴 High (small-cap discount) | 🟢 Low (mid-cap stability) | Axis |
| **DPU Volatility Risk** | 🟡 Higher (small base, one tenant risk) | 🟢 Lower (182 tenants, diversified) | Axis |
| **Acquisition Risk** | 🟢 Lower (one deal moves needle) | 🟡 Higher (execution volume) | Atrium |
| **Yield Comp Risk** | 🟢 Higher yield (~6.7%) | 🟡 Lower yield (~6.4%) | Atrium |
| **Transparency Risk** | 🔴 **High (WALE, tenant % unknown)** | 🟢 Low (4.4yr WALE, 46.7% disclosed) | **Axis** |
| **Shariah Compliance** | 🔴 No | 🟢 **Yes (Islamic REIT)** | **Axis** |
| **Manager Governance** | 🔴 **Poor** | 🟢 **Excellent** | **Axis** |
| **Manager Ownership Concentration** | 🔴 **59% family control** | 🟢 **Dispersed, professional** | **Axis** |
| **Management Stability** | 🟡 **Disrupted (CEO retired Aug 2024)** | 🟢 **Stable (19-year CEO tenure)** | **Axis** |
| **CEO Industry Leadership** | 🟡 **None** | 🟢 **MRMA Chairman, Best CEO awards** | **Axis** |

### 7.8 Manager Comparison: Atrium vs Axis REIT Managers

| Factor | Atrium REIT Managers | Axis REIT Managers (ARMB) | Winner |
|--------|---------------------|-------------------------|--------|
| **Ownership** | Chan Kam Tuck 59% via Glory Blitz | Dispersed, professional structure | **Axis** |
| **Major Unitholder** | Chan Kam Tuck ~23.7% (conflict risk) | EPF, KWAP, Public Mutual ~50%+ (institutional) | **Axis** |
| **CEO Tenure** | Acting since Sept 2024 (interim) | Leong Kit May 19 years (since 2006) | **Axis** |
| **CFO Tenure** | Lee Hooi Hoong 17+ years (dual role ⚠️) | Nikki Ng 17 years (dedicated CFO) | **Axis** |
| **Recent Changes** | ⚠️ CEO retired Aug 2024 (family connection) | ✅ No changes, stable leadership | **Axis** |
| **Board Independence** | 50% (3/6) | Multiple INEDs with committee chairs | **Axis** |
| **Committees** | 2 only (Audit, Nominating) | **4 (Audit, Nomination, Remuneration, Executive)** | **Axis** |
| **Performance Fees** | ❌ Not disclosed | ✅ Yes - tied to NPI | **Axis** |
| **Fee Increase Protection** | ❌ Not specified | ✅ Requires 2/3 unitholder approval | **Axis** |
| **Industry Leadership** | 🟡 None documented | ✅ MRMA Chairman, Best CEO awards | **Axis** |
| **Succession Planning** | 🔴 Unclear (Acting CEO) | ✅ Clear (planned COO→CEO 2016) | **Axis** |

**Manager Verdict:** Axis REIT Managers demonstrates **superior governance practices** across all dimensions - ownership structure, management stability, board independence, fee alignment, and industry recognition. Atrium REIT Managers has significant governance concerns around concentrated family control and recent management disruption.

### 7.6 Investment Profile Match

#### Choose Atrium REIT (5130) If:
- ✅ You prioritize higher dividend yield (~6.7% vs ~5.3%)
- ✅ You accept higher risk for potential higher returns
- ✅ You believe in Klang Valley industrial concentration
- ✅ You want exposure to ASA5's 2026 full-year contribution
- ✅ You can tolerate limited analyst coverage and liquidity
- ✅ You have confidence in management's AEI strategy
- ⚠️ **You accept that ~21% of rental income comes from a tenant (Lumileds) whose parent is in selective default (S&P 'SD' rating)**

#### Choose Axis REIT (5106) If:
- ✅ You prioritize **capital preservation and stability**
- ✅ You want **superior risk management** (33% gearing, 3.8-4.0x coverage, 65% fixed-rate)
- ✅ You need **interest rate protection** (vs Atrium's ~93% floating exposure)
- ✅ You value **transparency** (4.4yr WALE, tenant % disclosed)
- ✅ You want **diversified geographic exposure** (6 states vs 2 regions)
- ✅ You need **Shariah compliance** for Islamic investment mandates
- ✅ You require **institutional-grade liquidity** and research coverage
- ✅ You want a **20-year track record** through multiple market cycles
- ✅ You prefer **higher absolute DPU** (10.55 sen vs 9.30 sen)

### 7.7 Peer Comparison Verdict

| Category | Winner | Margin | Key Factor |
|----------|--------|--------|------------|
| **Scale & Diversification** | Axis REIT | **Massive** | 69 vs 9 properties; 182 vs undisclosed tenants |
| **Financial Strength** | Axis REIT | **Significant** | 33% vs 43.5% gearing; 3.8-4.0x vs 2.09x coverage |
| **Interest Rate Protection** | Axis REIT | **Massive** | 65% vs ~7% fixed; hedging vs none |
| **Income Stability** | Axis REIT | **Significant** | 4.4yr WALE known vs undisclosed |
| **Transparency** | Axis REIT | **Significant** | Discloses WALE, tenant % vs data gaps |
| **Tenant Credit Risk** | Axis REIT | **Critical** | Diversified vs Lumileds ~21% + S&P 'SD' parent |
| **Operational Efficiency** | Atrium REIT | **Narrow** | 100% vs 94% occupancy |
| **Absolute DPU** | Axis REIT | **Moderate** | 10.55 sen vs 9.30 sen (+13.4%) |
| **DPU Growth Momentum** | Atrium REIT | **Moderate** | +22.4% vs +13.8% YoY |
| **NAV-Based Yield** | Atrium REIT | **Narrow** | ~6.7% vs ~6.2% (+0.5pp) |
| **Market Yield (Actual April 2026)** | Atrium REIT | **Significant** | **7.27% vs 5.15% (+2.12pp)** |
| **Shariah Compliance** | Axis REIT | **Binary** | Yes vs No |
| **Risk-Adjusted Returns** | Axis REIT | **Significant** | Lower risk, strong returns |
| **Institutional Suitability** | Axis REIT | **Significant** | Liquidity, 20yr track record, compliance |
| **Value/Small Investor** | Atrium REIT | **Moderate** | Higher yield, potential upside, discount |

**Critical Differential - Interest Rate Risk:**
Axis REIT's **65% fixed-rate debt** with 10-year sukuk at 4.00% vs Atrium REIT's **~93% floating-rate exposure with NO hedging** represents the most material risk distinction. In a rising rate environment, this alone could swing profitability by millions of RM annually.

**Critical Differential - Tenant Credit Risk:**
Atrium REIT's **~21% rental income concentration from Lumileds** - whose parent Signify is in **S&P selective default ('SD')** - represents a existential threat not present in Axis REIT's diversified 182-tenant base.

**Overall Assessment:**

**Axis REIT (5106) is the CLEAR WINNER for:**
- ✅ Risk-averse, institutional, or stability-focused investors
- ✅ Islamic investors (Shariah compliance)
- ✅ Those prioritizing interest rate protection (65% fixed)
- ✅ Investors requiring transparency and professional risk management
- ✅ Long-term, buy-and-hold strategies

**Atrium REIT (5130) suits ONLY:**
- ⚠️ Risk-tolerant, yield-focused investors
- ⚠️ Those with deep conviction in Klang Valley industrial demand
- ⚠️ Investors who can accept ~21% income from a tenant with distressed parent
- ⚠️ Tactical traders seeking potential NAV discount recovery
- ⚠️ Those comfortable with ~93% floating-rate debt exposure

**Bottom Line:** Axis REIT delivers **higher absolute DPU (10.55 vs 9.30 sen)** with **substantially lower risk** (33% vs 43.5% gearing, 3.8-4.0x vs 2.09x coverage, 65% vs ~7% fixed-rate). 

**Yield Reality Check:**

**Actual Market Yields (April 2026):**
- Atrium REIT: **7.27%** (at RM 1.280 share price)
- Axis REIT: **5.15%** (at RM 2.050 share price)
- **Difference: +2.12 percentage points** in Atrium's favor

**Why the Large Yield Gap?**
The ~2.1pp yield advantage is **compensation for risks**, not free returns:

| Factor | Atrium REIT | Axis REIT |
|--------|-------------|-----------|
| **Market Valuation** | 8% discount (P/B 0.92x) | 21% premium (P/B 1.21x) |
| **Market Cap** | RM 340M (small-cap) | RM 4.15B (mid-cap) |
| **Liquidity Risk** | 🔴 High | 🟢 Low |
| **Concentration Risk** | 🔴 9 properties, Lumileds ~21% | 🟢 69 properties, 182 tenants |
| **Interest Rate Risk** | 🔴 ~93% floating, no hedging | 🟢 65% fixed, active hedging |
| **Transparency Risk** | 🟡 WALE, tenant % unknown | 🟢 Disclosed (4.4yr, 46.7%) |

**Bottom Line:** The 7.27% yield is **risk-adjusted compensation** - investors demand higher returns for bearing higher risks. Whether that compensation is adequate depends on individual risk tolerance.

**For most investors, Axis REIT's lower risk profile justifies its slightly lower yield.**

---

## 8. Risk Analysis

### 8.1 Key Risk Factors
| Risk Category | Severity | Description | Mitigation |
|---------------|----------|-------------|------------|
| **Concentration Risk** | 🔴 High | Only 9 properties [T:513]; single tenant loss = significant impact | Diversified tenant base; long-term leases |
| **Geographic Risk** | 🟡 Medium-High | 78% of portfolio in Klang Valley [T:514] | Industrial demand supported by Port Klang |
| **Interest Rate Risk** | 🔴 **High** | **~93% floating-rate debt, NO hedging instruments** [T:515] | Conservative gearing; monitor BNM OPR |
| **Refinancing Risk** | 🟢 Low | RM 999 million MTN programme available [T:516] | Conservative gearing provides headroom |
| **Tenant Credit Risk** | 🟡 **Medium** | MNC tenant base; **⚠️ Lumileds parent in selective default (S&P 'SD')** [T:517] | 15-year lease provides some protection |
| **Tenant Concentration Risk** | 🔴 **High** | **Lumileds = ~21% of rental income (RM 10.7M)** [T:518]; single tenant loss = severe impact | Long-term lease reduces rollover risk |
| **Liquidity Risk** | 🟡 Medium | Smaller market cap vs peers [T:519] | Sufficient trading volume for retail investors |
| **Limited Analyst Coverage** | 🟡 Medium | Few research reports [T:520] | Monitor Bursa announcements directly |
| **Data Transparency Risk** | 🟡 Medium | WALE, lease expiry, top tenants not disclosed [T:521] | Available disclosures show stability |
| **Manager Governance Risk** | 🔴 **High** | **59% family control [T:522], recent CEO retirement [T:523], Acting CEO dual role** [T:524] | Maybank Trustees provides some oversight |
| **Related Party Risk** | 🔴 **High** | **Chan Kam Tuck controls Manager + ~24% unitholding [T:525]; former CEO was his brother** [T:526] | Bursa Malaysia related party rules |

### 8.2 Risk Mitigation Measures
1. **Active Asset Management:** RM 34.44 million capex in 2025 [T:527] maintains property competitiveness
2. **Conservative Leverage:** 43.5% gearing [T:528] provides 16.5% headroom [T:529] below regulatory limits
3. **Long-term Lease Strategy:** Secured rental income with built-in escalations [T:530]
4. **Quality Tenants:** MNC focus reduces default risk [T:531]

---

## 9. Outlook and Investment Considerations

### 8.1 Positive Factors
- ✅ **Full Occupancy:** 100% occupancy [T:532] provides stable income base
- ✅ **2024 Renewal Success:** 100% retention rate [T:533] with positive rental reversions (ASA2, ASA3, USJ Block B) [T:534]
- ✅ **Quality Tenant Base:** **6 of 9 tenants identified** [T:535] - all are MNC logistics/e-commerce (DHL, DSV, Lazada, Röhlig, Samsung SDS) [T:536]
- ✅ **ASA5 Contribution:** Full year revenue from ASA5 in 2026 (new 3-year lease tenant secured) [T:537]
- ✅ **DPU Recovery:** 22.4% YoY growth [T:538] signals operational improvement
- ✅ **Conservative Balance Sheet:** 43.5% gearing [T:539] with adequate interest coverage [T:540]
- ✅ **Industrial Sector Tailwinds:** Malaysia's logistics and manufacturing demand remains robust [T:541]
- ✅ **AEI Strategy Success:** ASA4 (DHL) and ASA5 (undisclosed) both secured tenants post-enhancement [T:542]

### 8.2 Areas to Monitor
- 🔴 **Lumileds Credit Risk:** Parent company (Lumileds Holding B/V) in selective default (S&P 'SD' rating) [T:543]. Monitor restructuring developments and any impact on Malaysia subsidiary operations
- ⚠️ **Tenant Concentration:** Lumileds contributes ~21% of rental income (RM 10.7M/year) [T:544]. Any operational/financial issues would severely impact Atrium REIT
- ⚠️ **Asset Age Concentration:** 44% of portfolio from 2007 IPO [T:545] (now ~18-36 years old [T:546]); Bayan Lepas 1 is 54 years old (built 1972) [T:547]. Monitor AEI needs and capex requirements for aging assets
- ⚠️ **Interest Coverage:** 2.09x [T:548] is adequate but at lower end of comfort zone
- ⚠️ **Single Asset Impact:** With only 9 properties [T:549], single tenant/Asset events have outsized impact
- ⚠️ **Interest Rate Sensitivity:** Monitor borrowing cost trends in 2026 [T:550]

### 8.3 Comparative Context (vs Axis REIT 5106.KL)
| Metric | Atrium REIT (5130) | Axis REIT (5106) |
|--------|-------------------|------------------|
| **Portfolio Size** | 9 properties [T:551] | 69 properties [T:552] |
| **Total Assets** | RM 723 million [T:553] | RM 5.26 billion [T:554] |
| **Gearing** | 43.5% [T:555] | ~33% [T:556] |
| **Interest Coverage** | ~2.09x [T:557] | ~3.2x [T:558] |
| **Occupancy** | 100% [T:559] | ~97% [T:560] |
| **2025 DPU** | 9.30 sen [T:561] | ~9.2 sen [T:562] |
| **Asset Age Profile** | 44% from 2007 IPO [T:563]; Bayan Lepas 1 = 54 years old [T:564] | More diversified; active capital recycling [T:565] |
| **Risk Profile** | Higher concentration [T:566] | More diversified [T:567] |

**Assessment:** Atrium REIT offers a higher-risk, higher-concentration play compared to Axis REIT. The 100% occupancy and recent ASA5 contribution provide near-term income stability, but the smaller portfolio size means less diversification.

---

## 10. Summary Assessment

### 10.1 Asset Quality: **GOOD with IMPROVED VISIBILITY** ⭐⭐⭐⭐
- 100% occupancy [T:568] demonstrates strong asset quality and tenant demand
- **6 of 9 tenants identified** (67% coverage) [T:569] - all are MNC logistics/e-commerce [T:570]
- 2024 renewal activity: **100% retention with positive rental reversions** [T:571]
- **Asset age profile:** 44% from 2007 IPO vintage (~18-36 years old) [T:572] + 4 properties with recent AEIs (2016-2025) [T:573] resetting effective age
- ⚠️ **Oldest asset:** Bayan Lepas 1 (built 1972, **54 years old**) [T:574] - potential future capex consideration
- ✅ **AEI strategy:** 4 of 9 properties (44%) enhanced since 2016 [T:575], effectively modernizing key assets
- RM 34.44 million capex investment in 2025 [T:576] maintains competitiveness
- Prime industrial locations in Klang Valley and Penang [T:577]
- MNC tenant base (DHL, DSV, Lazada, Röhlig, Samsung SDS, Lumileds) [T:578] provides credit quality
- ⚠️ **Remaining Gaps:** WALE not disclosed [T:579]; ASA5 tenant identity undisclosed [T:580]; ASA1 & USJ Block A/C current tenants unclear [T:581]
- ⚠️ **Credit Concern:** Lumileds (~21% of income) [T:582] has parent in selective default (S&P 'SD') [T:583]

### 10.2 Debt Sustainability: **ADEQUATE with CAVEATS** ⭐⭐⭐
- Gearing ratio of 43.5% [T:584] is conservative and well within limits
- Interest coverage of ~2.09x [T:585] meets minimum thresholds but warrants monitoring
- 68.6% of debt matures 2027-2029 [T:586]; no near-term refinancing cliffs
- RM 999 million MTN programme [T:587] offers refinancing flexibility
- ⚠️ **CRITICAL RISK:** ~93% floating-rate debt with **NO hedging instruments** [T:588] - highly exposed to BNM OPR changes

### 10.3 Dividend Sustainability: **GOOD** ⭐⭐⭐⭐
- 22.4% YoY DPU growth to 9.30 sen [T:589] demonstrates recovery
- 100% occupancy [T:590] provides stable rental income base
- Conservative payout ratio (~29.9% of realised income) [T:591]
- ASA5 full-year contribution from 2026 [T:592] supports future distributions

---

## 11. Sources and References

### Primary Sources - Official Documents
| Source | Type | Reference | Direct Link |
|--------|------|-----------|-------------|
| **2025 Annual Report & CG Report** [T:593] | Official Filing | DCS-12032026-00030 [T:594], March 26, 2026 [T:595] | [Bursa Malaysia Announcements](https://www.bursamalaysia.com/market_information/announcements/company_announcements) - Search "ATRIUM" or "5130" |
| **Q4 2025 Unaudited Results** [T:596] | Quarterly Filing | Announced January 29, 2026 [T:597] | [Bursa Malaysia - Company Announcements](https://disclosure.bursamalaysia.com/FileAccess/apbursaweb/download?id=226376) |
| **Atrium REIT Official Website** [T:598] | Company IR | - | https://www.atriumreit.com.my |
| **Atrium REIT Investor Relations** [T:599] | Financial Reports | Annual Reports, Quarterly Reports | https://www.atriumreit.com.my/investor-relations/ |

### Key 2025 Announcements (Bursa Malaysia)
| Date | Announcement | Reference |
|------|--------------|-----------|
| **March 26, 2026** [T:600] | 2025 Annual Report & CG Report | DCS-12032026-00030 [T:601] |
| **January 29, 2026** [T:602] | Q4 2025 Unaudited Financial Results | DCS-29012026-00029 [T:603] |
| **December 30, 2025** [T:604] | Fourth Interim Income Distribution | DCS-30122025-00099 [T:605] |
| **November 27, 2025** [T:606] | Third Interim Income Distribution | DCS-27112025-00078 [T:607] |
| **September 8, 2025** [T:608] | **MTN Tranche 5A Issuance (RM 18.84m)** [T:609] | DCS-08092025-00045 [T:610] |
| **August 2025** [T:611] | ASA5 Tenancy Commencement [T:612] | Various property updates |
| **May 2025** [T:613] | Q1 2025 Results & Second Interim Distribution | Quarterly filing |

### Peer REIT References
| REIT | Stock Code | Website | Notes |
|------|------------|---------|-------|
| **Axis Real Estate Investment Trust** | 5106.KL | https://www.axis-reit.com.my | Primary Malaysian industrial REIT peer for comparison |
| **Mapletree Logistics Trust** | MLT (SGX) | https://www.mapletreelogisticstrust.com | Asia Pacific logistics REIT benchmark |

### Industry Research & Benchmark Reports
| Source | Type | URL |
|--------|------|-----|
| **Mapletree Logistics Trust** | Asia Pacific Logistics REIT Benchmark | [Independent Market Research Report 2024-25](https://investor.mapletreelogisticstrust.com/misc/independent-market-research-report24-25.pdf) |
| **Asia Pacific Real Assets Association (APREA)** | Industry Association | https://www.aprea.asia |
| **JLL Malaysia** | Industrial Market Research | https://www.jll.com.my |
| **Savills Malaysia** | Property Market Reports | https://www.savills.com.my |
| **CBRE Asia Pacific** | Industrial Research | https://www.cbre.com |

### Data Aggregators
| Source | URL | Notes |
|--------|-----|-------|
| **ChartNexus** | https://www.chartnexus.com | Historical price and financial data |
| **KLSE Screener** | https://www.klsescreener.com/v2/stocks/5130 | Real-time quotes and financials |
| **MarketScreener** | https://www.marketscreener.com/quote/stock/ATRIUM-REAL-ESTATE-INV-200227262/ | Analyst research and estimates |
| **Simply Wall St** | https://simplywall.st/stocks/my/reits/5130-atrium | Analysis and visualization |

### News and Analysis
| Source | URL | Notes |
|--------|-----|-------|
| **The Edge Malaysia** | https://www.theedgemalaysia.com | Tenant announcements, industry news |
| **The Edge Property** | https://www.edgeprop.my | Property transactions, tenant updates |
| **The Star (Business)** | https://www.thestar.com.my/business | Financial results, market updates |
| **Bernama (Business)** | https://www.bernama.com/business | Official announcements |

### Tenant Research Sources (Supplemental)
| Source | URL | Tenant Data Identified |
|--------|-----|------------------------|
| **ChartNexus** | https://www.chartnexus.com | Lumileds, Samsung SDS, DSV, DHL |
| **Value Invest Asia** | https://valueinvestasia.com | Lumileds lease details |
| **Hartamas** | — | Röhlig renewal details |
| **Bursa Malaysia** | https://www.bursamalaysia.com | ASA5 acquisition, AEI completion |
| **Atrium REIT Annual Reports** | https://www.atriumreit.com.my | Historical tenant lists |

**Note on Tenant Identification:**
Atrium REIT does not publicly disclose tenant names in Bursa announcements due to confidentiality agreements. Tenant identities were compiled from:
- Annual Report portfolio disclosures
- Property news websites (The Edge, EdgeProp)
- Historical tenant lists from prior annual reports
- Industry research sources (ChartNexus, Value Invest Asia)
- Google web search for "Atrium REIT" + tenant/property names

### Regulatory Framework
| Source | URL |
|--------|-----|
| **Securities Commission Malaysia REIT Guidelines** | https://www.sc.com.my/regulation/guidelines |
| **Bursa Malaysia Listing Requirements** | https://www.bursamalaysia.com/regulation |

### How to Access the 2025 Annual Report
1. **Via Bursa Malaysia:**
   - Go to https://www.bursamalaysia.com
   - Navigate to: Market Information → Announcements → Company Announcements
   - Search for "ATRIUM" or stock code "5130"
   - Filter by date: March 26, 2026
   - Download PDF: "Annual Report & CG Report - 2025"

2. **Via Atrium REIT Website:**
   - Go to https://www.atriumreit.com.my
   - Navigate to: Investor Relations → Financial Reports → Annual Reports
   - Download "Annual Report 2025"

---

## 12. Important Disclaimers

### Not Investment Advice
This analysis is prepared for **informational and educational purposes only**. It does not constitute investment advice, an offer, or a solicitation to buy or sell any securities. Always conduct your own research and consult a licensed financial advisor before making investment decisions.

### Data Limitations
- Financial data is compiled from publicly available sources and may contain errors or omissions
- Some metrics (particularly interest coverage and WACD) are derived/calculated from available data and should be verified against the official Annual Report
- **Granular debt maturity by specific year** (2026, 2027, 2028, 2029+) is not fully disclosed in quarterly filings and requires full Annual Report Notes
- **Top tenant percentages and WALE** are **not publicly disclosed** by Atrium REIT (standard for small industrial REITs; larger REITs like MLT typically disclose more)
- **Exact fixed vs floating rate debt split** requires Annual Report Note 15/16 "Borrowings" section
- **Industry benchmark data** sourced from Mapletree Logistics Trust report reflects a large-cap Asia Pacific logistics REIT and may not be directly comparable to Atrium REIT's small-cap, Malaysia-focused industrial portfolio
- Market prices and yields change daily; verify current values before any transaction

### Forward-Looking Statements
Any forward-looking statements in this report are based on publicly available information and do not constitute forecasts or guarantees of future performance.

### Date of Analysis
This report reflects data as of **December 31, 2025** (FY 2025) [T:614] with the Annual Report published **March 26, 2026** [T:615]. Financial conditions and metrics may have changed since this date.

---

**Report Prepared:** April 2026 [T:616]
**Data Coverage:** FY 2025 (January 1 - December 31, 2025) [T:617]
**Next Expected Update:** Q1 2026 results (around May 2026) [T:618]
