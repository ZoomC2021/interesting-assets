  ...
  return (
    <>
      ...
      <div
        ...
        className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-white shadow-panel z-50 flex flex-col transform transition-transform duration-300 ease-out forced-colors:border-l-2 forced-colors:border-black"
        tabIndex={-1}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-200 bg-neutral-50">
          ...
          <button
            onClick={handleClose}
            className="p-2 text-neutral-400 hover:text-neutral-600 hover:bg-neutral-100 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2"
        ...
        {citations.length > 0 && (
          <div className="px-6 py-3 border-b border-neutral-200 bg-white">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-xs font-medium text-neutral-600 uppercase">Filter:</span>
              <button
        ...
        {filteredCitations.length > 0 && (
          <div className="px-6 py-2 border-b border-neutral-200 bg-white">
            <button
              ...
              className="text-xs flex items-center gap-1.5 text-primary-600 hover:text-primary-700 font-medium px-3 py-1.5 rounded-md hover:bg-primary-50 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500"
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {citations.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-neutral-500">No citations available</p>
            </div>
          ) : filteredCitations.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-neutral-500">No citations match the selected filter</p>
              <button
                onClick={() => handleFilterChange('all')}
            ...
            <div className="space-y-4" role="list" aria-label="Citations list">
              {filteredCitations.map((citation, idx) => {
                ...
                return (
                  <div
                    key={citation.id}
                    role="listitem"
                    className={`bg-neutral-50 rounded-lg p-4 border border-neutral-200 transition-all ${
                      isHovered ? 'ring-2 ring-primary-200 border-primary-300' : ''
                    }`}
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {/* Source Group Badge + Time Sensitive Warning */}
                        <div className="flex flex-wrap gap-2 text-xs mb-2">
                          <span className={`px-2 py-1 rounded ${groupConfig.colorClass}`}>
                            {groupConfig.label}
                          </span>
                          {citation.timeSensitive && (
                            <span 
                              className="px-2 py-1 bg-warning-100 text-warning-700 rounded flex items-center gap-1"
                        ...
                        {/* Citation Text */}
                        <p className="text-xs text-neutral-500 mt-2 bg-white p-2 rounded border border-neutral-100">
                          <span className="font-medium">Citation:</span> {citation.citation}
                        </p>

                        {/* Date Accessed */}
                        <p className="text-xs text-neutral-400 mt-1">
                          <span className="font-medium">Accessed:</span> {formatDate(citation.dateAccessed)}
                        </p>
                        ...
                        {citation.url && (
                          <a
                            ...
                            className="inline-flex items-center gap-1 text-xs text-primary-600 hover:text-primary-700 mt-2 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-1 rounded px-1 py-0.5"
        ...
        <div className="border-t border-neutral-200 px-6 py-4 bg-neutral-50">
          <div className="flex items-center justify-between">
            <p className="text-xs text-neutral-500">
              Sources verified from official annual reports and Bursa Malaysia announcements.
            </p>
